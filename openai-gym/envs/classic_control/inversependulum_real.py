...
#        assert self.action_space.contains(action), "%r (%s) invalid"%(action, type(action))
        ...
        self.state = np.array([x, x_dot, theta, theta_dot])
                ...
                or x > self.x_threshold or (np.abs(theta_dot) > np.pi*2 and np.abs(theta) < 0.05)
        ...
        if not done:
            if(np.abs(theta) < 0.2):
                ...
                reward = ((np.pi - np.abs(theta)) / (np.pi)) * 4
        else:
            reward = -10.
